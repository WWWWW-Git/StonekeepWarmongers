...
//Tries to equip the mannequin.
/obj/structure/mannequin/proc/tryEquip(mob/user)
	if(unchangeable)
		return
	var/obj/item/item_in_hand = usr.get_active_held_item()
	var/item_slot = MannequinEquipHelper(user.zone_selected)
	if(!item_in_hand)
		...
		if(clothing[item_slot])
			if(canEquip(user ,item_in_hand))
				if(user.dropItemToGround(item_in_hand))
					var/obj/item/I = clothing[item_slot]
					MannequinUnequip(null,item_slot)
...
*/
/obj/structure/mannequin/proc/canEquip(mob/user, obj/item/item_to_check)
	var/selected_area = EquipHelper(user.zone_selected)
	...
	if(user)
		to_chat(user, "<span class='warning'>\The [item_to_check] doesn't fit there.</span>")
	return FALSE
