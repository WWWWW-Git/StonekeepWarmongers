...
/datum/browser/modal/input_list/New(mob/user, message, title, list/items, default, timeout = 0)
	...
	for(var/item in items)
		...
		choices += {"
			<label>
				<input type='radio' name='selectedbutton' value='[html_encode(form_key)]' required [NULLABLE("[default]" == "[item]") && "checked"] />
				[item]
			</label>"}
	...
	<form style="width: 100%; height: 100%;" action="byond://">
		...
		"<input style='margin: 0.25em 0; z-index: 5' id='searchbar' type='text' placeholder='Find...' stealfocus/>"]
...
/datum/browser/modal/input_list/set_selectedbutton(selectedbutton)
	. = ..()
	if(!(selectedbutton in results))
		CRASH("user gave impossible response [selectedbutton]")

	src.selectedbutton = results[selectedbutton]
