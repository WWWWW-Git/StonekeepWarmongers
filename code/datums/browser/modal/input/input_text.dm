...
/datum/browser/modal/input_text
	/// If TRUE, this input will encode its selectedbutton upon being set.
	var/encode = TRUE
...
/datum/browser/modal/input_text/New(mob/user, message, title, default, max_length, multiline, encode, timeout)
	...
	<form style="display: flex; flex-direction: column; height: 100%;" action="byond://">
		...
		<textarea
			...
			stealfocus>[html_encode(default)]</textarea>
...
/datum/browser/modal/input_text/set_selectedbutton(selectedbutton)
	. = ..()
	var/processed_choice = encode ? html_encode(selectedbutton) : selectedbutton
	src.selectedbutton = trim(processed_choice, PREVENT_CHARACTER_TRIM_LOSS(max_length))
