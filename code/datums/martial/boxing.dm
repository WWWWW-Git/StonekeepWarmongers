...
/datum/martial_art/boxing/harm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if(!damage)
		...
		D.visible_message("<span class='warning'>[A]'s [atk_verb] misses [D]!</span>", \
						"<span class='danger'>I avoid [A]'s [atk_verb]!</span>", "<span class='hear'>I hear a swoosh!</span>", COMBAT_MESSAGE_RANGE, A)
		to_chat(A, "<span class='warning'>My [atk_verb] misses [D]!</span>")
	...
	D.visible_message("<span class='danger'>[A] [atk_verb]ed [D]!</span>", \
					"<span class='danger'>You're [atk_verb]ed by [A]!</span>", "<span class='hear'>I hear a sickening sound of flesh hitting flesh!</span>", COMBAT_MESSAGE_RANGE, A)
	to_chat(A, "<span class='danger'>I [atk_verb]ed [D]!</span>")
	...
	if(D.getStaminaLoss() > 50 && istype(D.mind?.martial_art, /datum/martial_art/boxing))
		...
		if((D.stat != DEAD) && prob(knockout_prob))
			D.visible_message("<span class='danger'>[A] knocks [D] out with a haymaker!</span>", \
							"<span class='danger'>You're knocked unconscious by [A]!</span>", "<span class='hear'>I hear a sickening sound of flesh hitting flesh!</span>", COMBAT_MESSAGE_RANGE, A)
			to_chat(A, "<span class='danger'>I knock [D] out with a haymaker!</span>")
			D.apply_effect(200,EFFECT_KNOCKDOWN,armor_block)
			D.SetSleeping(100)
			D.say(GLOB.hit_appends)
